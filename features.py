...
def compute_curvature(points, k=10):
    ...
    n = len(points)
    k = min(k, n - 1)
    D = _pairwise_sq(points, points)                    # (n, n)
    curvatures = np.zeros(n)
    for i in range(n):
        nn = np.argpartition(D[i], k + 1)[: k + 1]     # k neighbours + self
        nbrs = points[nn]
        if len(nbrs) < 3:
            continue
        cov = np.cov(nbrs.T)
...
def compute_descriptors(points, kp_idx, k=30):
    ...
    to the *k* nearest points.  Because only distances are used, the descriptor
    is invariant to rotation and reflection.
    """
    k = min(k, len(points) - 1)
    kp_pts = points[kp_idx]
    D = _pairwise_sq(kp_pts, points)                     # (n_kp, n_pts)
    # sort each row, skip column 0 (self-distance ≈ 0)
    D_sorted = np.sort(D, axis=1)[:, 1: k + 1]
    descs = np.sqrt(D_sorted)                             # Euclidean distances
    return descs
...
def rotation_search(
    ...
    # Pre-compute target squared-norm columns for fast NN
    # For each candidate rotation we need min-over-target for every src point.
    # With small point counts after coarse downsampling this is fast.

    def _score(angle_rad):
        ca, sa = np.cos(angle_rad), np.sin(angle_rad)
        R = np.array([[ca, -sa], [sa, ca]])
        rotated = src_c @ R.T + mu_t                     # rotate then shift to target centroid
        D = _pairwise_sq(rotated, tgt)                    # (n_src, n_tgt)
